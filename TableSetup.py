...
dep_subjects = {'Language': {'WRI', 'HIST', 'ENGLISH', 'JAPANESE', 'SPANISH', 'GASP'}, 'Natural Sciences': {'BIO', 'MATH', 'CHEM', 'PHYS', 'PSYCH', 'COGS'}, 'Engineering': {'CSE', 'ENGR'}}
...
['Intro to Physics', 'Optics', 'Chaotic Systems', 'Physics Group Study', 'Intermediate Physics', 'Nuclear Physics', 'Quantum Physics'],
...
def Drop_Tables():
    ...
    tables = ['departments', 'classes', 'teachers', 'students', 'enrollments', 'admins']
...
def assign_teacher(dep_name, teachers):
    ...
    while(True):
        s = select(teachers).where(teachers.c.department == dep_name, teachers.c.num_classes == i)
        try:
    ...
    query = query.where(teachers.columns.teacher_ID == list(conn.execute(s).first())[0])
    conn.execute(query)
...
def Initialize_Departments():
    ...
    for i, department in enumerate(dep_subjects):
        query = db.insert(departments).values(dep_ID = i + 1, dep_name = department)
        conn.execute(query)
...
def Initialize_Teachers():
    ...
    for i, teacher in enumerate(instructors):
        ...
        query = db.insert(teachers).values(teacher_ID = str(9000 + i), name = teacher, password = generate_password(), date_of_birth = generate_DOB('teacher'), department = dep_choice, num_classes = 0)
        conn.execute(query)
...
def Initialize_Classes(teachers):
    ...
    classes = db.Table('classes', metadata,
        ...
        db.Column('num_students_enrolled', db.Integer(), nullable=False),
    ...
    for i in range(len(course_dic)):
        for j, course in enumerate(course_des[i]):
            ...
            query = db.insert(classes).values(course_ID= str(course_dic[i] + "_" + str((j*10))), course_Des = course, dep_name = dep_name , teacher_ID = teacher_ID , num_students_enrolled = 0, capacity = random.randint(4, 10) * 5, times = str(meets[temp][0] + " " + meets[temp][random.randint(1, len(meets[temp]) - 1)]))
            conn.execute(query)
...
def Initialize_Students():
    ...
    students = db.Table('students', metadata,
        db.Column('student_ID', db.String(255), nullable=False),
        db.Column('name', db.String(255), nullable=False),
        db.Column('password', db.String(255), nullable=False),
        db.Column('date_of_birth', db.String(255), nullable=False)
        )
    ...
    for i, stud_name in enumerate(student):
        query = db.insert(students).values(student_ID = str(1000 + i), name = stud_name, password = generate_password(), date_of_birth = generate_DOB('student'))
        conn.execute(query)
        ...
        for i in range(num_classes):
            ...
            while True:
                ...
                if potential_course not in stud_courses and course_count[potential_course] < course_limit[potential_course] and course_date[potential_course] not in stud_times:
                    ...
                    query = db.insert(enrollments).values(student_ID = stud_ID, course_ID = potential_course, grade = randint(60, 100))
                    conn.execute(query)

                    course_count[potential_course] += 1

                    # Update the Student Count in the class
                    query = db.update(classes).values(num_students_enrolled = course_count[potential_course])
                    query = query.where(classes.columns.course_ID == potential_course)
                    conn.execute(query)
...
def Initalize_Admins():
    ...
    for i, admin in enumerate(admin_names):
        ...
        query = db.insert(admins).values(admin_ID = str(10000 + i), name = admin, password = generate_password(), date_of_birth = generate_DOB('teacher'))
        conn.execute(query)
